...
const { OPTION_CATALOG, getOptionMetadata, getCompatibility } = require("../option-catalog");
const { lintDocument } = loadExtension();

function lint(option, flavor = "generic", version = "") {
  return lintDocument(document(`[mysqld]\n${option}`), { ...lintOptions, target: { flavor, version } });
}
...
test("catalog preserves metadata and supports normalized and loose names", () => {
  ...
  assert.equal(getOptionMetadata("performance_schema_consumer_events_stages_current").type, "boolean");
...
test("version checks cover introduction, deprecation and removal boundaries", () => {
  assert.equal(lint("log-replica-updates=ON", "mysql", "8.0.25")[0].code, "unsupported-option");
  ...
  assert.equal(lint("innodb-log-file-size=64M", "mysql", "8.0.30")[0].code, "deprecated-option");
  assert.equal(lint("mysql-native-password=ON", "mysql", "8.4").length, 0);
  assert.equal(lint("mysql-native-password", "mysql", "9.0")[0].code, "removed-option");
  assert.equal(lint("innodb-buffer-pool-instances=2", "mariadb", "10.5.1")[0].code, "deprecated-option");
  assert.equal(lint("innodb-buffer-pool-instances=2", "mariadb", "10.6")[0].code, "removed-option");
});
...
test("generic, unspecified and malformed versions do not invent version warnings", () => {
  ...
  assert.equal(lint("loose-mysql-native-password=ON", "mysql", "9.0").length, 0);
  assert.equal(lint("userstat=ON", "mysql")[0].code, "unsupported-option");
  assert.equal(lint("userstat=ON", "mariadb", "10.11").length, 0);
  assert.equal(getCompatibility(undefined, { flavor: "mysql", version: "8.4" }), undefined);
});
...
test("catalog drives numeric limits, signed integers, sizes and enums", () => {
  ...
  assert.equal(lint("innodb-redo-log-capacity=4M", "mysql", "8.4")[0].code, "out-of-range");
  assert.equal(lint("innodb-redo-log-capacity=1G", "mysql", "8.4").length, 0);
  assert.equal(lint("innodb-flush-log-at-trx-commit=3", "mysql")[0].code, "invalid-enum");
